...
const loginWithCompanyRole = async (page: Page, SSN?: string) => {
    ...
    await page.getByRole('button', { name: 'Select Registered community role and authorize mandate' }).click();
...
const startNewApplication = async (page: Page, applicationName: string) => {
    ...
    await page.locator('#block-servicepageauthblock').getByRole('link', { name: 'New application' }).click();
}
