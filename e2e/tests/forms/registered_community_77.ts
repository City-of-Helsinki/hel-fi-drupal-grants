...
const formPages: PageHandlers = {
  ...
  '2_avustustiedot': async (page: Page, {items}: FormPage) => {
    ...
    if (items['edit-start-subvention-applied']) {
      await page.locator('#edit-start-subvention-applied')
        .getByText(items['edit-start-subvention-applied'].value ?? '').click();
    }
