...
class JsonMapper {
  ...
  private function setTargetValue(array &$data, string $targetPath, string|array $sourceValue, array $definition): void {
    ...
    match($definition['mapping_type']) {
      ...
      'file', 'multiple_files' => $targetValue = $sourceValue,
      default => $targetValue['value'] = $sourceValue,
    };
  ...
  public function mapFiles(array $dataSources): array {
    $fileData = [];
    $singleFileDefinitions = array_filter($this->mappings, fn(array $item) => $item['mapping_type'] === 'file');

    foreach ($singleFileDefinitions as $targetPath => $definition) {
      $this->handleFile($fileData, $definition, $targetPath, $dataSources);
    }

    $multiFileDefinitions = array_filter($this->mappings, fn(array $item) => $item['mapping_type'] === 'multiple_files');
    foreach ($multiFileDefinitions as $targetPath => $definition) {
      $this->handleMultipleFiles($fileData, $definition, $targetPath, $dataSources);
    }
  ...
  /**
   * Get all required field values for file. Can also handle multiple files.
   *
  ...
  private function getFileData(array $defaultData, array $sourceData, string $sourcePath): array {
    ...
    $ready = [];
    // Loop through possibly multiple files and all the fields.
    foreach ($files as $file) {
      ...
      $fieldNames = isset($file['integrationID']) ? $defaultFieldsForFile : $defaultFieldsForNoFile;
      ...
      foreach ($fieldNames as $fieldName) {
        ...
        if (isset($file[$fieldName]) && $fieldName !== 'fileType') {
          ...
          if ($defaultData[$fieldName]['valueType'] === 'bool') {
            $field['value'] = isset($file[$fieldName]) && $file[$fieldName] ? 'true' : 'false';
          }
          elseif ($defaultData[$fieldName]['ID'] === 'integrationID' && $file[$fieldName]) {
            $field['value'] = '';
            ...
            $field['value'] = isset($file[$fieldName]) ? (string) $file[$fieldName] : $val;
          }
        }
